...
def generate():
    task_data = []
    row_x = ['X']
    znak = rd.randint(0, 1) * 2 - 1
    ax = rd.randint(1, 10) * znak
    znak = rd.randint(0, 1) * 2 - 1
    bx = rd.randint(1, 10) * znak
    for i in range(4):
        item = rd.randint(-5, 5)
        item = item * ax + bx
        row_x.append(item)
    task_data.append(row_x.copy())

    row_y = ['Y']
    znak = rd.randint(0, 1) * 2 - 1
    a = rd.randint(1, 10) * znak
    znak = rd.randint(0, 1) * 2 - 1
    b = rd.randint(1, 10) * znak
    for i in range(4):
        noise = rd.randint(-5, 5)
        item = (task_data[i+1] * a + b) * (1+noise*0.01)
        row_y.append(item)
    task_data.append(row_y.copy())
